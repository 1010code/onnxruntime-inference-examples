...
{
    Ort::Env env;
    const char *model_path = "./data/tf_model.onnx";
    // mingw64 必須將字元陣列轉成寬字元才能編譯過
    int length = MultiByteToWideChar(CP_ACP, 0, model_path, -1, NULL, 0);
    std::wstring wideModelPath = std::wstring(model_path, model_path + length);
    const wchar_t *model_path_ = wideModelPath.c_str();
    Ort::Session session{env, model_path_, Ort::SessionOptions{}};
    ...
    // 設定 input data
    std::vector<float> input_values = {6.7f, 3.1f, 4.4f, 1.4f};
    // 配置 tensors 空間
